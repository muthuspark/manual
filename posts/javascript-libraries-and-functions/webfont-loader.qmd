---
title: "WebFont Loader - A Developer's Handbook"
...
After installing Web Font Loader using your preferred method (see "Installation"), include the script in your HTML file.  It's generally recommended to place it in the `<head>` section, before the closing `</head>` tag, to ensure it loads early. If using the direct download or CDN method, the inclusion will look like this (replace with the correct path if you downloaded the file):
