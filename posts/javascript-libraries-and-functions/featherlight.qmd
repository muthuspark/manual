---
title: "Featherlight - A Developer's Handbook"
...
1. **Download:** Download the latest version of Featherlight from [link to download - replace with actual link].  Include the `featherlight.min.js` and `featherlight.min.css` files in your project's `<head>` section.

2. **CDN:**  Use a CDN link to include Featherlight in your project.  You can find suitable CDN links on sites like jsDelivr or cdnjs. (Example - replace with actual CDN link):

```html
<link rel="stylesheet" href="https://cdn.example.com/featherlight.min.css">
<script src="https://cdn.example.com/featherlight.min.js"></script>
```
...
Please use the issue tracker on [link to issue tracker - replace with actual link] to report bugs.  Before creating a new issue, search existing issues to see if the problem has already been reported.
...
Submit feature suggestions through the issue tracker on [link to issue tracker - replace with actual link], using the "Feature Request" label.
